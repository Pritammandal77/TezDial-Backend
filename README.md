...
- Delete a listing — protected by a bcrypt-hashed security pin set at creation time
...
- The `pin` field is bcrypt-hashed before saving and never returned in plaintext.
- Deleting a listing requires the correct pin, verified via `business.ispinCorrect()`.

## Deployment

Deployed on Render. Build command: `npm install`. Start command: `npm run start`.
